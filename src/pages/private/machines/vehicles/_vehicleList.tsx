...
export default function VehiculeList() {
  ...
  const MACHINE_CATEGORY = "vehicule"
  ...
  function detailsLink(isEdit: boolean, uid: string): string {
    return isEdit ?
      "./detailsVehicle?" + "isEdit=true&id=" + uid :
      "./detailsVehicle?" + "isEdit=false&id=" + uid
  }
              ...
              <DropdownMenuTrigger asChild>
                <div className="flex">
                  <h3 className={`text-xl font-bold tracking-tight ${(item.deletedAt === null) ? '' : DELETED_STYLE}`}>
                    {UpperFirstLetter(item.brand) + " " + UpperFirstLetter(item.model) + (!item.seatCount ? "" : (" - " + item.seatCount + " places"))}
                  </h3>
  ...
  return (
    <>
      <div className="flex items-center my-4">
        <h1 className="text-lg font-semibold md:text-2xl">Vehicules</h1>
      </div>
      ...
      <div className="flex flex-col pt-4 gap-4 grid border-t border-dashed">
        <div className="flex">
          ...
          <div className="flex flex-col items-center text-center w-1/2">
            <Button className="flex items-center text-center w-full" onClick={() => addLink()}> <SquarePlus className="mr-2" /> Ajouter un véhicule </Button>
          </div>
        </div>


        <div className="flex items-center">
          <h1 className="text-lg font-semibold md:text-2xl">Liste des véhicules : </h1>
        </div>
