...
export default function TrailerList() {
  ...
  const MACHINE_CATEGORY = "remorque"
  ...
  function onChangeFilter(e: any) {
    ...
    if (e.target.value === "") {
      ...
      setDisplayListFiltered(displayList.filter((obj: any) =>
        ...
        (!!obj.length && obj.length.toString().includes(e.target.value.toLowerCase())) ||
  ...
  const addLink = () => {
    navigate("./detailsTrailer")
  }

  function detailsLink(isEdit: boolean, uid: string): string {
    return isEdit ?
      "./detailsTrailer?" + "isEdit=true&id=" + uid :
      "./detailsTrailer?" + "isEdit=false&id=" + uid
  }
  ...
  function renderItemList(displayList: any): React.ReactElement[] {
              ...
              <DropdownMenuTrigger asChild>
                <div className="flex">
                  <h3 className={`text-xl font-bold tracking-tight ${(item.deletedAt === null) ? '' : DELETED_STYLE}`}>
                    {item.brand.toUpperCase() + " " + UpperFirstLetter(item.model) + (!item.length ? "" : " - " + item.length + "m")}
                  </h3>
    ...
    else { element.push(<div key="0" className="flex flex-1 p-4 mb-4 rounded-lg border border-dashed">Aucune remorque !</div>) }
    return element
  ...
  return (
    <>
      <div className="flex items-center my-4">
        <h1 className="text-lg font-semibold md:text-2xl">Remorques</h1>
      </div>
      ...
      <div className="flex flex-col pt-4 gap-4 grid border-t border-dashed">
        <div className="flex">
          ...
          <div className="flex flex-col items-center text-center w-1/2">
            <Button className="flex items-center text-center w-full" onClick={() => addLink()}> <SquarePlus className="mr-2" /> Ajouter une remorque </Button>
          </div>
        </div>


        <div className="flex items-center">
          <h1 className="text-lg font-semibold md:text-2xl">Liste des remorques : </h1>
        </div>

        <div className="flex flex-1 gap-2 grid">
