        ...
        if (flag.current === false) {
            ...
            vehicleTypeService.getAll()
                .then(res => {
                    ...
                    setVehicleTypesDisplay(commonDetails.PopulateVehicleTypes(res.data.data, MACHINE_CATEGORY, "Aucune remorque pour le moment !"))
        ...
        <>
                    ...
                    <BreadcrumbList>
                        ...
                        <BreadcrumbItem>
                            {((isEdit === "true" || isEdit === "false") && id !== null && !isNaN(parseInt(id))) ?
                                <Link to="" className="font-semibold text-2xl text-white">{editableState === true ? "Modifier une remorque" : "Consulter une remorque"} </Link> :
                                <Link to="" className="font-semibold text-2xl text-white">Ajouter une remorque </Link>
                            }
                        </BreadcrumbItem>
                    </BreadcrumbList>
                </Breadcrumb>
            ...
            <div className="border p-10 border-solid rounded-lg shadow-sm">
                ...
                {(id === null || isNaN(parseInt(id))) ?
                    ...
                    <>
                        <h2 className="text-2xl font-bold tracking-tight">
                            {UpperFirstLetter(vehicleData.brand) + " " + vehicleData.model + " - " + vehicleData.length + "m :"}
                        </h2>
                        <p className="text-sm text-muted-foreground">
                ...
                <form id="mainForm" onSubmit={onSubmit}>
                    ...
                    <div className="flex flex-1 justify-center">
                        ...
                        <div className="flex flex-1 gap-x-10 flex-col gap-4 grid grid-cols-1 lg:grid-cols-2">
                                            ...
                                            <SelectTrigger>
                                                <SelectValue
                                                    placeholder={vehicleTypes.length === 0 ?
                                                        UpperFirstLetter(vehicleTypes.find((obj: any) => obj.id === parseInt(vehicleData.typeId)).type)
                                                        : " "}
                                                />
                                            </SelectTrigger>
                            ...
                            <div className="grid gap-2">
                                <Label htmlFor="axleCount">Nombre d'essieux</Label>
                                <Input
                                    {...editableState === true ? { disabled: false } : { disabled: true }}
                                    name="axleCount"
                                    value={vehicleData.axleCount}
                                    onChange={onChange}
                                    type="text" />
                            </div>
